...
# @FileName: yolo_infer_v4_tts.py
# @Time: 2025/7/8 09:37:36
# @Author: MBAS
# @Project: SafeYolo
# @Function: Integrate all previous functions + add beautification and TTS features
...
from utils.paths import LOGS_DIR, CHECKPOINTS_DIR, YOLOSERVER_ROOT
...
def parser_args():
    ...
    parser.add_argument("--model", type=str,
                        default=r"E:\SafeYolo\yolo_server\models\checkpoints\train11-20250710-115632-yolo11n-best.pt",
                        help="模型权重文件")
    parser.add_argument("--source", type=str,
                        default="demo.mp4", help="输入源（摄像头ID或视频路径）")
    ...
    parser.add_argument("--tts_duration", type=int, default=16, help="语音合成时长")
    parser.add_argument("--tts_interval", type=int, default=16, help="语音间隔")
    parser.add_argument("--tts_text", type=str,
                        default="您已进入施工场所，请规范佩戴安全帽！",
                        help="语音合成内容")
...
def main():
    ...
    logger = setup_logging(
        base_path=LOGS_DIR,
        log_type="infer",
        model_name="train11-20250710-115632-yolo11n-best.pt" # 示例: "train11-20250710-115632-yolo11n-best.pt"
    ...
    # 6. Calculate beautification parameters
    beautify_params = calculate_beautify_params(
        ...
        label_mapping=yaml_config.get("beautify_settings", {}).get("label_mapping", {}),
        color_mapping=yaml_config.get("beautify_settings", {}).get("color_mapping", {})
    )
    ...
    if source.isdigit() or source.endswith((".mp4", ".avi", ".mov", ".mkv")):
        ...
        logger.info(f"YOLO参数: {yolo_args}")

        for idx, result in enumerate(model.predict(**vars(yolo_args))):
            # Initialize save paths on first frame
            if idx == 0:
