...
if __name__ == "__main__":
    # 定义测试用的日志目录（当前目录下的test_logs文件夹）
    test_logs_dir = Path(__file__).parent / "test_logs"
    logger = setup_logging(base_path=test_logs_dir, log_type="train_test", model_name='yolo8n', temp_log=True)
    rename_log_file(logger, 'train1', model_name='yolo8n')