...
def init_tts() -> pyttsx3.Engine | None:  # 用法
    """
    初始化语音合成引擎
    :return: 返回语音合成引擎
    """
...
def process_tts_detection(result, tts_enable, tts_duration,
    ...
    if not tts_enable or not tts_engine:
        logger.info("语音合成引擎未加载或语音合成引擎初始化失败")
        return
    classes = result.boxes.cls.cpu().numpy() if result.boxes else []
    # 检查是否有佩戴安全帽的检测结果
    has_person = 2 in classes
    has_head = 0 in classes
    has_safety_helmet = 4 in classes
    # 获取当前时间
    current_time = time.time()
    # 检查是否处于冷却时间,具体上次提示小于冷却时间则不提示
    in_cooldown=tts_state.get('last_tts_time') and (current_time -tts_state.get('last_tts_time') < tts_duration)
